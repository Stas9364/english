...
### Шаг 5a. Варианты для Dropdown in gaps

Если у страницы тип **Dropdown in gaps**, у каждого вопроса текст с пропусками `[[]]` и блоки **Options for gap 1**, **Options for gap 2** и т.д.

- Для каждого пропуска задаёте список **вариантов**, которые пользователь увидит в выпадающем списке. Отметьте чекбоксом **Correct** у тех вариантов, которые считаются правильными для этого пропуска (можно несколько).
- Кнопка **Add option for gap N** добавляет новый вариант в соответствующий пропуск. Иконка корзины удаляет вариант после подтверждения. В каждом пропуске должен быть **хотя бы один вариант** и **хотя бы один отмечен как Correct**.

### Шаг 5b. Пары для Matching
