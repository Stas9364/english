...
3. **Example (optional)** — образец выполнения задания, который показывается пользователю на этой странице. Удобно, когда нужно наглядно показать, как отвечать: например, «I usually get up at 7 a.m.» для страницы с составлением предложений.
4. **Questions** — список вопросов на этой странице (см. ниже).

В шапке карточки страницы **слева** — кнопки со стрелками **←** и **→** (меняют порядок страниц; подсказки при наведении: **Move page left** / **Move page right**). Справа — **удаление страницы**. Новый порядок применяется сразу в форме и сохраняется при нажатии **Create quiz** / **Save changes**. Стрелка влево недоступна для первой страницы, вправо — для последней.

Иконка корзины **удаляет страницу сразу после подтверждения** (запись удаляется в БД). Удалить можно только если страниц больше одной.
...
- Блок **Pages** — как при создании: тип страницы, заголовок, пример, вопросы; для каждого типа — свои поля (варианты, правильные ответы для Text input, варианты по пропускам для Dropdown in gaps, пары «справа / слева» для Matching).
- Стрелки **←** / **→** слева в шапке карточки страницы **меняют порядок** страниц в квизе (подсказки при наведении). Справа — удаление страницы. Изменение в форме сразу; сохраняется по **Save changes**.
...
| Образец для страницы | Example (optional) | Нет |
| Порядок страниц | Стрелки ← / → слева в шапке карточки страницы | Нет |
...
В правом нижнем углу админки есть плавающая кнопка с иконкой чата — она открывает полноэкранное окно **админ‑чата с ИИ**. Чат можно использовать как помощника по настройке квизов, формулировке заданий и т.п.

### Как хранится история чата

- История сообщений **сохраняется только локально в браузере** (в `localStorage`) на вашем устройстве.
- Сообщения **не попадают в базу данных** проекта и не синхронизируются между устройствами или браузерами.
- Если вы откроете админку в другом браузере или на другом компьютере, там будет **своя пустая история**.
- После обновления страницы история чата на этом же устройстве **сохраняется**, пока вы её явно не очистите.

### Как очистить историю

- В шапке окна чата есть кнопка с иконкой корзины **«Очистить чат»**.
- Нажатие этой кнопки **полностью очищает историю** локально (все сохранённые сообщения удаляются из `localStorage`).
- Очистка не влияет на другие аккаунты/браузеры — каждый админ чистит свою локальную историю отдельно.

### Как остановить текущий ответ

- Во время генерации ответа рядом с полем ввода появляется кнопка **«стоп»** (иконка квадрата).
- Если нажать её, текущий запрос к модели **немедленно прерывается**:
  - уже полученный текст ответа останется в окне;
  - недополученная часть ответа не будет загружена;
  - можно сразу задать новый вопрос.

### Лимиты по запросам к модели

- Для защиты от перегрузки и неожиданных счетов у чата есть ограничения:
  - **не более 30 запросов в минуту**;
  - **не более 14 000 запросов в сутки** на весь проект (совокупно по всем администраторам).
- Если лимиты будут превышены, возможны ошибки при запросах (например, временные отказы в генерации ответа).
- Рекомендуется:
  - не нажимать отправку слишком часто подряд;
  - при необходимости длинного диалога формулировать вопросы чуть крупнее, а не спамить десятками почти одинаковых запросов.
