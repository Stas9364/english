...
export type EditPageBlockFormValues = {
  pages: {
    type: TestType;
    title?: string;
    questions: {
...
export interface EditPageBlockProps {
  form: UseFormReturn<EditPageBlockFormValues>;
  pageIndex: number;
  defaultOption: (o?: { id?: string; option_text: string; is_correct: boolean }) => EditPageBlockFormValues["pages"][0]["questions"][0]["options"][0];
  defaultQuestion: (q?: EditPageBlockFormValues["pages"][0]["questions"][0] | undefined, orderIndex?: number) => EditPageBlockFormValues["pages"][0]["questions"][0];
  onRemove: () => void;
  canRemove: boolean;
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    {pageType === "input" || pageType === "select_gaps" ? (
                      <textarea
                        {...form.register(
                          `pages.${pageIndex}.questions.${qIndex}.question_title`
                        )}
                        ...
                        className={cn(
                          ...
                          form.formState.errors.pages?.[pageIndex]?.questions?.[qIndex]
                            ?.question_title && "border-destructive"
                        )}
                      />
                    ) : (
                      <Input
                  ...
                  <div className="space-y-2 border-t border-border/60 pt-4">
                    <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                      ...
                      {pageType === "input" || pageType === "select_gaps"
                        ? " — use 1:, 2:, … for each gap"
                        : ""}
                    </Label>
                    <textarea
                      {...form.register(
                        `pages.${pageIndex}.questions.${qIndex}.explanation`
                      )}
                      placeholder={
                        pageType === "input" || pageType === "select_gaps"
                  ...
                  {(pageType === "single" || pageType === "multiple") && (
                    <QuizQuestionChoiceOptions
                      ...
                      defaultOption={() => {
                        ...
                        return { option_text: opt.option_text, is_correct: opt.is_correct };
                      }}
              ...
              <Button
                ...
                onClick={() =>
                  questionsArray.append(
                    ...
                    ) as EditPageBlockFormValues["pages"][0]["questions"][0]
                  )
                }
              >
