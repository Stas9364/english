...
function buildGeneratePrompt(params: GenerateQuizPagesParams): string {
  ...
  const inputContractionRule = `  - CONTRACTION RULE (MANDATORY when generating JSON): for every accepted correct answer you put in "options", if the text can appear with a contraction OR as the equivalent full form, you MUST return BOTH as separate option objects with is_correct=true and the same gap_index. Do NOT return only one form — both are stored as correct answers in the quiz. Works both ways: "doesn't" requires "does not", and "does not" requires "doesn't". Apply to every contraction in the phrase (doesn't/does not, didn't/did not, I'm/I am, I'll/I will, we're/we are, can't/cannot, won't/will not, etc.). Example for full_answer: options must include BOTH { "option_text": "I'll do it", "is_correct": true, "gap_index": 0 } AND { "option_text": "I will do it", "is_correct": true, "gap_index": 0 }. Example for gaps: if gap 0 accepts "doesn't live", also add { "option_text": "does not live", "is_correct": true, "gap_index": 0 }. Never merge variants into one option_text string.`;
  const inputRules =
    inputMode === "gaps"
      ? [
          `- For type "input":`,
